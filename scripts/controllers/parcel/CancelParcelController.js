    ...
    ParcelService.getcalcelledParcelList(sessionStorage.getItem("UserId")).then(function (results) {
        if (results.status == 200) {
            ...
            for (i = 0; i < $scope.list.length; i++) {
                ...
                dTable.row.add([ $scope.mainlist[i].ParcelID, $scope.mainlist[i].source, $scope.mainlist[i].destination, $scope.mainlist[i].type == 'E' ? 'Envelope' : $scope.mainlist[i].type == 'B' ? 'Box' : $scope.mainlist[i].type == 'P' ? 'Packet' : $scope.mainlist[i].type, $scope.mainlist[i].weight + " kg.", $scope.mainlist[i].transemail, $scope.mainlist[i].receiveremail, $scope.mainlist[i].statusdescription]).draw();
            }
        }
    });
